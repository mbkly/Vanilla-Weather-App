...
function handlePositionFromCoordinates(position) {
...
function currentLocation() {
  navigator.geolocation.getCurrentPosition(handlePositionFromCoordinates);
}
...
function getWeatherFromSearchForm(event) {
  event.preventDefault();
  let cityInput = document.querySelector("#city-name");
  let newCityvalue = cityInput.value;
  let newCity = document.querySelector("h1");
  newCity.innerHTML = newCityvalue;
  getWeatherByCity(getCityFromInput());
}
  ...
  dailyForecast.forEach(function (forecastDay, index) {
    if (index < 4) {
      forecastHTML =
        forecastHTML +
        `
    <div class="col" id= "forecast-container">
